...
public class WaitingRoomResource {
    ...
    public WaitingRoomResource(WaitingRoomService waitingRoomService, WaitingRoomRepository waitingRoomRepository) {
    ...
    public ResponseEntity<List<WaitingRoomDTO>> getAllWaitingRooms(
        ...
        log.debug("REST request to get a page of WaitingRooms");
        Page<WaitingRoomDTO> page;
        if (eagerload) {
            page = waitingRoomService.findAllWithEagerRelationships(pageable);
        } else {
            page = waitingRoomService.findAll(pageable);
        }
